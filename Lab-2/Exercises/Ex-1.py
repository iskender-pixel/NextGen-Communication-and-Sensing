...
def plotReflection(Zl, Z0, fRange):
    ...
    plt.plot(fRange, gamma_squared)
    plt.xlabel('Frequency (GHz)', fontsize=15)
    plt.ylabel('Power reflection coefficient' ,fontsize=15)
    plt.grid(True)
...
def plotRadiatedPower(Zl, Z0, fRange, P_in):
    ...
    tau_squared = (1-gamma_squared)*P_in

    plt.figure(figsize=(8, 5))
    plt.rcParams['font.size'] = '16'
    plt.plot(fRange, tau_squared)
    plt.xlabel('Frequency (GHz)', fontsize=15)
    plt.ylabel('Radiated power', fontsize=15)
    plt.tick_params(axis='both', labelsize=15)
    plt.grid(True)
